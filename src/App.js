...
function App() {
  ...
  const [quizzes, setQuizzes] = useState(() => {
    try { return JSON.parse(localStorage.getItem('quiz-master-quizzes')) || []; } catch { return []; }
  });
  ...
  const finishQuiz = (attempt) => {
    setQuizzes(prev => prev.map(q => q.id === activeQuiz.id ? { ...q, attempts: [...(q.attempts || []), attempt] } : q));
    setActiveQuiz({ ...activeQuiz, attempts: [...(activeQuiz.attempts || []), attempt], lastAttempt: attempt });
    setPage('results');
  };
  ...
  const content = useMemo(() => {
    ...
    if (page === 'manage') return <ManagePage quizzes={quizzes} onBack={() => setPage('home')} onOpen={(q) => { setActiveQuiz(q); setPage('quiz'); }} onDelete={(id) => setQuizzes(prev => prev.filter(q => q.id !== id))} />;
  ...
  return <div className={dark ? 'app dark' : 'app'}>
    <header className="topbar">
      <button className="brand" onClick={() => { setActiveQuiz(null); setPage('home'); }}><span className="brand-mark">Q</span><span>Quiz Master</span></button>
      <div className="top-actions"><button className="icon-btn" onClick={() => setDark(v => !v)} aria-label="Toggle theme">{dark ? '☀' : '☾'}</button><button className="avatar">KC</button></div>
    </header>
    <main>{content}</main>
    <footer>Quiz Master · Build quizzes. Learn better.</footer>
  </div>;
}

function HomePage({ quizzes, onCreate, onManage, onSample }) {
  return <section className="hero page">
    <div className="eyebrow">AI-POWERED STUDY TOOL</div>
    <h1>Turn your notes into<br /><span>better quizzes.</span></h1>
    <p className="hero-copy">Upload a PDF, choose your settings, and create a focused multiple-choice quiz in seconds.</p>
    <div className="hero-actions"><button className="primary" onClick={onCreate}>＋ Create a quiz</button>{quizzes.length > 0 && <button className="secondary" onClick={onManage}>Manage quizzes <span>({quizzes.length})</span></button>}</div>
    <div className="feature-grid">
      <Feature icon="↥" title="Upload a PDF" text="Use lecture notes, reviewers, or study materials." />
      <Feature icon="✦" title="Generate questions" text="Create clear multiple-choice questions from your material." />
      <Feature icon="◷" title="Test yourself" text="Set a timer and see your score when you're done." />
    </div>
    {quizzes.length === 0 && <div className="sample-card"><div><strong>Want to try it first?</strong><p>Start with a built-in sample quiz to see the experience.</p></div><button className="text-btn" onClick={onSample}>Try sample →</button></div>}
  </section>;
}
...
function CreatePage({ onBack, onCreate }) {
  const [file, setFile] = useState(null); const [count, setCount] = useState(10); const [minutes, setMinutes] = useState(10); const [error, setError] = useState(''); const [busy, setBusy] = useState(false);
  const create = async () => {
    if (!file) return setError('Please choose a PDF first.');
    setBusy(true); setError('');
    try {
      const text = await file.text();
      const response = await fetch('/api/generate-quiz', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text: text.slice(0, 30000), count }) });
      if (!response.ok) throw new Error('The AI service is not configured yet. You can still try the sample quiz.');
      const data = await response.json();
      if (!Array.isArray(data.questions) || !data.questions.length) throw new Error('No questions were generated.');
      onCreate({ id: crypto.randomUUID(), title: file.name.replace(/\.pdf$/i, ''), questions: data.questions, timeLimit: minutes, createdAt: new Date().toISOString(), attempts: [] });
    } catch (e) { setError(e.message); } finally { setBusy(false); }
  };
  return <section className="page narrow"><button className="back" onClick={onBack}>← Back</button><div className="section-head"><div className="eyebrow">NEW QUIZ</div><h2>Create a quiz</h2><p>Give Quiz Master your study material and choose how you want to practice.</p></div>
    <label className="upload"><input type="file" accept="application/pdf,.pdf" onChange={e => setFile(e.target.files?.[0] || null)} /><div className="upload-icon">↑</div><strong>{file ? file.name : 'Choose a PDF file'}</strong><span>{file ? `${Math.round(file.size / 1024)} KB selected` : 'Click to browse from your computer'}</span></label>
    <div className="settings"><Setting label="Questions" value={count} min={3} max={30} onChange={setCount} /><Setting label="Time limit" value={minutes} min={1} max={60} suffix="min" onChange={setMinutes} /></div>
    {error && <div className="error">{error}</div>}<button className="primary full" disabled={!file || busy} onClick={create}>{busy ? 'Generating…' : 'Generate quiz'}</button>
    <p className="privacy">Your PDF is sent only to the configured AI endpoint when you generate a quiz.</p>
  </section>;
}
...
function QuizPage({ quiz, onBack, onFinish }) {
  const [index, setIndex] = useState(0); const [answers, setAnswers] = useState(() => Array(quiz.questions.length).fill(-1)); const [left, setLeft] = useState(quiz.timeLimit * 60); const [done, setDone] = useState(false);
